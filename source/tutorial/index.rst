...
如果你不熟悉 PHP，建议你先看看 `W3Schools PHP 教程 <https://www.w3schools.com/php/default.asp>`_，然后再继续阅读下面的内容。

在本教程中，你将创建一个 **基本的新闻应用程序**。你将从编写可加载静态页面的代码开始。接下来，你将创建一个新闻栏目，用于从数据库中读取新闻内容。最后，我们再编写一个表单，用于在数据库中创建新闻内容。
...
-  路由基础
-  表单验证
-  使用CodeIgniter的 ``Query Builder`` 执行基本数据库查询

整个教程分为几个章节，每个章节仅解释 CodeIgniter 框架功能的一小部分。你将会阅读到以下几个章节：

-  介绍章节, 当前章节, 本章将概括性的为你讲解如何下载框架并运行自带的默认应用。
-  :doc:`静态页面 <static_pages>`, 本章将为你讲解控制器，视图和路由的基础知识。
-  :doc:`新闻栏目 <news_section>`, 你将在这里开始使用模型，并将进行一些基本的数据库操作。
...
您可以从站点手动下载发行版，但是对于本教程，我们将使用推荐的方法并通过 `Composer` 安装 `AppStarter` 软件包。在命令行中输入以下内容：

::

    ➜ composer create-project codeigniter4/appstarter ci-news

这将创建一个新文件夹 ``ci-news``，其中包含你的应用程序代码，并且在 **vendor** 文件夹中安装了 CodeIgniter。
...
现在，在浏览器地址栏中输入正确的 URL，你将看到框架的欢迎页面。
...
除此之外，当你在程序中遇到异常或其他错误时，CodeIgniter 还会提供一些有用的错误页面。打开 ``app/Controllers/Home.php`` 并修改一些内容以生成一个错误（删除分号或花括号试试~）。屏幕将显示类似以下内容的页面：

.. image:: ../images/error.png

这里有几件事要注意:

1. 将鼠标悬停在顶部的红色标题上会显示一个 ``search`` 链接，该链接将在新标签页中打开 Google.com 并搜索此错误的相关信息。
